...
- `data/drafts/featured-drafts.json`: 메인 화면 카드뉴스 전용 데이터
- `data/drafts/archive-records.json`: 수집 보관함 전용 데이터
- `data/drafts/featured-curation.json`: 메인 화면에 노출되는 featured draft의 한국어 핵심 메시지 보정본
...
- 현재 프론트엔드는 메인에서 `featured-drafts.json`, 보관함에서 `archive-records.json`을 각각 읽고, `article-drafts.json`은 전체 묶음과 자동화 호환용으로 유지합니다.

이론 데이터 운영 방식:

- `data/theory-papers.json`은 내부 참고문헌 창고입니다. 화면에 모든 문헌을 노출하지 않습니다.
- `data/theory-notes.json`은 화면과 설명문 작성에 사용할 압축 정리본입니다.
- 공개 화면에서는 이론별 설명과 anchor work 3-5개만 보여주고, 전체 대표 문헌은 상세/참고문헌 영역에서만 사용합니다.
- 현재 노화 이론 섹션은 `theory-notes.json`과 `theory-papers.json`을 읽어 선택형 상세 패널을 렌더링합니다.

주의:

- 생성된 카드뉴스는 `summary_status: ai_draft` 상태입니다.
- 현재 스크립트는 OpenAI API를 호출하지 않고 규칙 기반 분류와 한국어 템플릿으로 초안을 만듭니다.
- 메인 화면의 카드뉴스 문구는 `featured-curation.json`이 있으면 이를 우선 적용해, 영어 초록 문장이 그대로 노출되지 않도록 합니다.
...
- `data/raw/crossref-latest.json`, `data/drafts/article-drafts.json`, `data/drafts/featured-drafts.json`, `data/drafts/archive-records.json` 업데이트
- 변경사항이 있으면 자동 커밋 및 push

## 다음 구현 과제

- 사람이 검수한 초안만 `data/articles.json`으로 승격하는 게시 스크립트
- 이론-논문 자동 연결 결과를 협업자 피드백으로 보정
- 초록 기반 카드뉴스 문장을 더 자연스러운 한국어로 바꾸는 OpenAI API 요약 단계
- draft preview/review 화면
- GitHub Actions 주간 실행 로그 확인과 실패 알림 보강
...
2026-06-20 현재 파일 기준 재생성 결과:

- generatedAt: `2026-06-15T02:54:08.309Z`
- selected window: 14일
- total collected: 151개
- auto publish: 65개
- needs review: 86개
- featured: 33개
- archive: 118개

## 2026-06-01 업데이트: 이론-논문 자동 연결

메인 페이지의 노화 이론 상세 패널에 `이 이론으로 읽을 최신 카드뉴스` 섹션을 추가했습니다.

- 연결 기준: 이론의 관련 분야, 독자 질문, 이론 키워드, key concept, OpenAlex 개념을 함께 점수화합니다.
- 연결 대상: 현재 메인에 노출되는 featured 카드뉴스입니다.
- 표시 내용: 카드뉴스 제목, 핵심 메시지, 연결 점수, 연결 이유입니다.
- 운영 방식: 규칙 기반 연결이므로 협업자 검토를 통해 어색한 매칭을 `THEORY_ARTICLE_RULES`와 `data/theories.json`의 키워드에서 보정합니다.
- 2026-06-17 점검에서 키워드/OpenAlex 근거가 최소 1개 있는 항목만 연결하도록 품질 기준을 강화했고, `access`, `function`처럼 너무 넓은 키워드를 더 구체적인 표현으로 바꿨습니다.
